...
class App {
      ...
      if (self.reticle.visible) {
        ...
        if (self.measurements.length == 2) {
          ...
          const distance = Math.round(
            self.getDistance(self.measurements) * 100
          );
          ...
          self.labels.push({
            div: text,
            point: self.getCenterPoint(self.measurements),
          });
  ...
  getHitTestResults(frame) {
    ...
    if (hitTestResults.length) {
      ...
      if (this.currentLine)
        this.updateLine(this.reticle.matrix, this.currentLine);
    } else {
  ...
  render(timestamp, frame) {
    ...
    this.labels.forEach((label) => {
      const pos = self.toScreenPosition(
        label.point,
        self.renderer.xr.getCamera(self.camera)
      );
